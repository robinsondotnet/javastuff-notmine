...
// Aquí se implementa la interface creada:
// En la palabra ClienteDAO que aparece en rojo, en el foquito, darle a que importe todos los abstractMethods.
public class ClienteDAO implements OperacionesBD<Cliente>{
    ...
    private static final String sql_update = "update cliente set nombre=?, apellidoPat=?, apellidoMat=?,"
                                             + "correo=?, celular=?, dni=? where id=?";
    ...
    @Override
    // Se usa el t que es de tipo cliente que tiene get and setler
    public boolean insert(Cliente t) {
        boolean resultado = false;
        try{
          pstm = con.getCon().prepareStatement(sql_insert);
          pstm.setString(1,t.getNombre()); // 1, 2 en la siguiente línea y los números de las demás filas, representan los símbolos 
          // de interrogación de más arriba
          ...
          //Update cuando se modifica el estado de base de datos, ejemplo cuando se actualiza, elimina un registro.
